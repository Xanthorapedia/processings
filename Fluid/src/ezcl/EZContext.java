...
public class EZContext {
	...
	/**
	 * Creates kernels specified by knlNaes form src code String. knlNames are
	 * the only kernels to be added and are optional. If no knlNames are present
	 * or knlNames = null, the context will add all the kernels. If there are
	 * valid knlNames present with at least 1 null in the name list, all kernels
	 * will be added, and the specified kernels will be created in addition. If
	 * a kernel name is preceded by a number, the context will create duplicates
	...
	public void addKernels(String[] src, String... knlNames) {
		if (knlNames == null || knlNames.length == 0)
			knlNames = null;
		else {
			// kernel names correction
			int totalKnls = 0;
			String[] duplicateNames = new String[knlNames.length];
			int[] precedingNums = new int[knlNames.length];
			for (int i = 0; i < knlNames.length; i++) {
				Object[] o = parseKernelName(knlNames[i]);
				// null kernel returns name of null and duplicate number of 0
				precedingNums[i] = (int) o[0];
				duplicateNames[i] = (String) o[1];
				// "kernel" should have duplicate number 1, while null
				// should have 0 even though they has the same preceding number
				if (precedingNums[i] == 0 && duplicateNames[i] != null)
					precedingNums[i]++;
				totalKnls += precedingNums[i];
			}
			// expand
			String[] tmp = new String[totalKnls];
			int k = 0;
			for (int i = 0; i < knlNames.length; i++)
				for (int j = 0; j < precedingNums[i]; j++) {
					tmp[k++] = duplicateNames[i];
				}
			knlNames = tmp;
		}
		...
		for (int i = 0; i < tmpKernels.length; i++) {
			int nameIndex = tmpKernels.length - i;
			String kernelName = null;
			if (nameIndex >= 0 && knlNames != null && nameIndex < knlNames.length)
				kernelName = knlNames[knlNames.length - nameIndex];
			// adds only valid kernels
			if (tmpKernels[i] != null || kernelName != null) {
				EZKernel newK = new EZKernel(tmpKernels[i], src, kernelName);
	...
	/**
	 * Sets the argIndex'th argument of a kernel named kernelName
	 * 
	 * @param kernelName
	 *            the name of the kernel being set
	 * @param argIndex
	 ...
	 *            an arguments
	 */
	public void setKernelArguments(String kernelName, int argIndex, Object argument) {
		getKernel(kernelName).setArgument(argument, argIndex);
	}
	...
	/**
	 * Gets the kernel by name. Preceding digits i (if there are) indicates the
	 * i'th kernel with this name.
	 * 
	 * @param name
	...
	public EZKernel getKernel(String name) {
		Object[] parse = parseKernelName(name);
		int index = (int) parse[0];
		String kernelName = ((String) parse[1]).trim();

		for (EZKernel knl : kernels)
			if (knl.getKernelName().trim().equals(kernelName))
				if (index == 0)
					return knl;
				else
					index--;
	...
	/**
	 * Parses the kernel name into name index and real name
	 * 
	 * @param name
	 *            the kernel name
	 * @return an array whose [0] is the duplicate index and [1] is the real
	 *         name
	 */
	private static Object[] parseKernelName(String name) {
		if (name == null)
		...
		while (Character.isDigit(name.charAt(i)))
			i++;
		if (i == 0)
			return new Object[] { 0, name.substring(i) };
		return new Object[] { Integer.parseInt(name.substring(0, i)), name.substring(i) };
	}
